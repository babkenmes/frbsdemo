...
router.post('/login', function (req, res, next) {
    passport.authenticate('local', function (err, user, info) {
		...
		var confData = {
			...
			"storageBucket": user.storageBucket,
			"messagingSenderId": user.messagingSenderId
		}
        res.json({ success: true, token: token, username: user.username, fullName: user.firstName + " " + user.lastName, role: user.role,confData:confData });
    })(req, res, next);
});
